...
describe Authem::Session do
  ...
  it 'generates secure token on creation' do
    expect(Authem::Token).to receive(:generate).exactly(:twice).and_return('a secure token')
    model = described_class.create(role: role, subject: user)
    expect(model.token).to eq('a secure token')
  end

  it 'generates secure client token on creation' do
    expect(Authem::Token).to receive(:generate).exactly(:twice).and_return('a secure token')
    model = described_class.create(role: role, subject: user)
    expect(model.client_token).to eq('a secure token')
  end
